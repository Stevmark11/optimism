...
type CrossSafeDeps interface {
	...
	// NotCrossSafe is called when a local block cannot be upgraded to cross-safe, and has to be dropped.
	...
	NotCrossSafe(chainID eth.ChainID, candidate types.DerivedBlockRefPair) error
}
...
func CrossSafeUpdate(logger log.Logger, chainID eth.ChainID, d CrossSafeDeps) error {
	...
	if errors.Is(err, types.ErrConflict) {
		...
		return d.NotCrossSafe(chainID, candidate)
	}
