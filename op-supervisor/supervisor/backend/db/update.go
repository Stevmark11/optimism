...
func (db *ChainsDB) NotCrossSafe(chainID eth.ChainID, candidate types.DerivedBlockRefPair) error {
	...
	// TODO: may have to change cross-unsafe block

	// Now invalidate the local-safe and cross-safe data.
	...
	db.emitter.Emit(superevents.NonCrossSafeEvent{
		ChainID:   chainID,
		Candidate: candidate,
	})
	return nil
...
func (db *ChainsDB) ReplaceBlock(chainID eth.ChainID, replacement eth.BlockRef, invalidated common.Hash) error {
	...
	crossSafeDB, ok := db.crossDBs.Get(chainID)
	if !ok {
		return fmt.Errorf("cannot find cross-safe DB of chain %s for block replacement work: %w", chainID, types.ErrUnknownChain)
	}
	...
	if err := crossSafeDB.ReplaceInvalidatedBlock(replacement, invalidated); err != nil {
		// TODO
	}
